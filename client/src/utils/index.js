...
export function createPrompt(form) {
	...
	//stars
	if (form.stars == 1) {
		starsValue = weatherData.stars[0];
	} else if (form.stars == 2) {
		starsValue = weatherData.stars[1];
	} else if (form.stars == 3) {
		starsValue = weatherData.stars[2];
	}
	...
	if (fogValue || clodyValue || moonValue) {
		return (
			'High quality astrophotography of a' +
			starsValue +
			' night sky,' +
			form.prompt +
			fogValue +
